...
class Permuntations2Test extends AnyFunSuite {
  ...
  test("Permute2 3") {
    val res = permutator.permuteUnique(Array(0, 1))
    compareIntListsIfEquals(List(List(0, 1), List(1, 0)), res)
  }

  test("Permute2 4") {
    val res = permutator.permuteUnique(Array(1))
    compareIntListsIfEquals(List(List(1)), res)
  }

  test("Permute2 5") {
    val res = permutator.permuteUnique(Array(1, 1, 2))
